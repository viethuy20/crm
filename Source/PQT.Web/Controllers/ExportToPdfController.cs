...
namespace PQT.Web.Controllers
{
    ...
    public class ExportToPdfController : Controller
    {
        ...
        {
            string pathForExport = UrlHelper.Root + "/Admin/" + "Report/" + modelName + actionName + "/" + itemId;
            byte[] fileContent = GeneratePdfFileWithParameters(pathForExport, parameter, pdfparameter);
            string attachment = "";
            if (fileContent != null)
            {

                var filename = modelName + "-" + actionName + "_" + DateTime.Now.ToString("yyyyMMddssfff") + ".pdf";
        ...
        public void PublishToPdfWithURL(string path, string name, string parameter = "")
        {
            ...
            if (fileContent != null)
            {
                attachment = "attachment; filename=" + name + "_" + DateTime.Now.ToString("yyyyMMddssfff") + ".pdf";
                Response.Clear();
                Response.AddHeader("content-disposition", attachment);
                Response.ContentType = "application/pdf";
                Response.BinaryWrite(fileContent);
        ...
        {
            string pathForExport = UrlHelper.Root + "/Admin/" + "Report/" + actionName;
            byte[] fileContent = GeneratePdfFileWithParameters(pathForExport, parameter);
            string attachment = "";
            if (fileContent != null)
            {
                if (string.IsNullOrEmpty(filename))
                    attachment = "attachment; filename=" + "_" + actionName + "_" + DateTime.Now.ToString("yyyyMMddssfff") + ".pdf";
                else
                {
                    attachment = "attachment; filename=" + filename + "_" + DateTime.Now.ToString("yyyyMMddssfff") + ".pdf";
                }
                Response.Clear();
                Response.AddHeader("content-disposition", attachment);
                Response.ContentType = "application/pdf";
        ...
        private byte[] ProcessingExport(string url, string pdfparameter = "")
        {
            ...
            var process = new Process
            {
                StartInfo =
                {
                    ...
                    Arguments = pdfparameter + " " + url + " " + filepath,
