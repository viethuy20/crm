...
namespace PQT.Web.Infrastructure.Helpers
{
    public class ExcelUploadHelper
    {
        ...
        public static string SaveFile(HttpPostedFileBase fileImport, FolderUpload folder)
        {
            ...
            var uploadPath = Path.Combine(HttpContext.Current.Server.MapPath("/data/uploads/" + folder), filename);
            try
            {
            ...
            {
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath("/data/uploads/" + folder));
                fileImport.SaveAs(uploadPath);
            }
            return uploadPath;
        }
        ...
        {
            return Path.Combine(HttpContext.Current.Server.MapPath("/data/uploads/" + folder), fileName);
        }
